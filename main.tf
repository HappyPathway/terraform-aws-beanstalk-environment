...
resource "aws_elastic_beanstalk_environment" "default" {
  ...
  setting {
    ...
    value     = var.elb_scheme == "internal" ? var.associate_public_ip_address : false
    resource  = ""
  }
  ...
  dynamic "setting" {
    for_each = var.ssh_listener_enabled ? ["*"] : []
    content {
      namespace = "aws:autoscaling:launchconfiguration"
      name      = "EC2KeyName"
      value     = var.keypair
